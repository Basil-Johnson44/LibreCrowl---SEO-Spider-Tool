...
from src.utils.paths import get_db_path, get_asset_dir

# Dynamic templates and static directories for standalone desktop app execution
...
@app.route('/api/ranks/check', methods=['POST'])
@login_required
def api_check_ranks():
    try:
        user_id = session.get('user_id')
        ...
        if not domain or not keywords:
            return jsonify({'success': False, 'error': 'Domain and keywords are required'})
            
        from src.core.rank_checker import check_keyword_rank
        from src.crawl_db import add_keyword_rank
        from src.settings_manager import SettingsManager
        
        tier = session.get('tier', 'guest')
        settings_mgr = SettingsManager(session_id=session.get('session_id'), user_id=user_id, tier=tier)
        settings = settings_mgr.get_settings()
        custom_browser_path = settings.get('customBrowserPath', '')
        ...
        for kw in keywords:
            ...
            if not kw:
                continue
            res = check_keyword_rank(domain, kw, region, custom_browser_path=custom_browser_path)
            
            # Formulate the rank value and details depending on success
            if res.get('success'):
                rank_num = res.get('rank')
                page_num = res.get('page')
                if isinstance(rank_num, int) or (isinstance(rank_num, str) and rank_num.isdigit()):
                    rank_val = f"{rank_num} (Page {page_num})"
                else:
                    rank_val = str(rank_num)
                ranking_url = res.get('ranking_url', '')
                google_url = res.get('google_url', '')
            else:
                rank_val = f"Failed: {res.get('error', 'Unknown failure')}"
                ranking_url = ''
                google_url = res.get('google_url', f"https://google.com/search?q={kw.replace(' ', '+')}")
                
            # Always save to database so history displays errors
            rank_id = add_keyword_rank(
                user_id=user_id,
                domain=domain,
                keyword=kw,
                region=region,
            ...
            res['id'] = rank_id
            res['keyword'] = kw
            res['checked_at'] = datetime.now().isoformat()
            res['rank'] = rank_val
            res['ranking_url'] = ranking_url
            res['google_url'] = google_url
            results.append(res)
...
@app.route('/api/ranks/history', methods=['GET'])
@login_required
def api_ranks_history():
    try:
        user_id = session.get('user_id')
        domain = request.args.get('domain')
        
        from src.crawl_db import get_keyword_ranks
        
        history = get_keyword_ranks(user_id, domain)
        return jsonify({'success': True, 'history': history})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/ranks/delete/<int:rank_id>', methods=['DELETE'])
@login_required
def api_ranks_delete(rank_id):
    try:
        user_id = session.get('user_id')
        
        from src.crawl_db import delete_keyword_rank
        
        success = delete_keyword_rank(user_id, rank_id)
        return jsonify({'success': success, 'message': 'Record deleted successfully' if success else 'Failed to delete record'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
